...
public static class IterablesUtils
{
    ...
    public static TItem[] Shuffle<TItem>(TItem[] array) => array.Length < 2 ? array : ShuffleArray(array);
    ...
    public static TItem[] Shuffle<TItem>(TItem[] array, int amount)
    {
        ...
        var targetAmount = result.Length;
        amount = 0;

        for (var i = array.Length - 1; amount < targetAmount; --i)
        {
