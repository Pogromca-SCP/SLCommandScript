...
public static class PlayerIterablesProvider
{
    ...
    public static IIterable AllPlayers() => new LazyListIterable<Player>(static () => AuthPlayers, LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all class d personnel.
    /// </summary>
    /// <returns>Iterable object for all class d personnel.</returns>
    public static IIterable AllClassDs() => new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.Team == Team.ClassD), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all scientists.
    /// </summary>
    /// <returns>Iterable object for all scientists.</returns>
    public static IIterable AllScientists() =>
        new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.Team == Team.Scientists), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all MTFs.
    /// </summary>
    /// <returns>Iterable object for all MTFs.</returns>
    public static IIterable AllMTFs() => new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.IsNTF), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all chaos insurgents.
    /// </summary>
    /// <returns>Iterable object for all chaos insurgents.</returns>
    public static IIterable AllChaos() => new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.IsChaos), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all SCPs.
    /// </summary>
    /// <returns>Iterable object for all SCPs.</returns>
    public static IIterable AllSCPs() => new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.IsSCP), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all flamingos.
    /// </summary>
    /// <returns>Iterable object for all flamingos.</returns>
    public static IIterable AllFlamingos() =>
        new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.Team == Team.Flamingos), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all humans.
    /// </summary>
    /// <returns>Iterable object for all humans.</returns>
    public static IIterable AllHumans() => new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.IsHuman), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all tutorials.
    /// </summary>
    /// <returns>Iterable object for all tutorials.</returns>
    public static IIterable AllTutorials() => new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.IsTutorial), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all spectators.
    /// </summary>
    /// <returns>Iterable object for all spectators.</returns>
    public static IIterable AllSpectators() =>
        new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.Role == RoleTypeId.Spectator), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all alive players.
    /// </summary>
    /// <returns>Iterable object for all alive players.</returns>
    public static IIterable AllAlive() => new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.IsAlive), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all disarmed humans.
    /// </summary>
    /// <returns>Iterable object for all disarmed humans.</returns>
    public static IIterable AllDisarmed() => new LazyListIterable<Player>(static () => AuthPlayers.Where(static p => p.IsDisarmed), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all NPCs.
    /// </summary>
    /// <returns>Iterable object for all NPCs.</returns>
    public static IIterable AllNPCs() => new LazyListIterable<Player>(static () => Player.GetAll(PlayerSearchFlags.RegularNpcs), LoadVariables);

    /// <summary>
    /// Retrieves iterable object for all dummies.
    /// </summary>
    /// <returns>Iterable object for all dummies.</returns>
    public static IIterable AllDummies() => new LazyListIterable<Player>(static () => Player.GetAll(PlayerSearchFlags.DummyNpcs), LoadVariables);
