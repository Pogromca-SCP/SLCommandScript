...
public class FileScriptsEventHandlers
{
    ...
    void OnPlayerMuted(Player player, Player target, bool isIntercom) =>
    ...
    void OnPlayerUnmuted(Player player, Player target, bool isIntercom) =>
    ...
    void OnCheckReservedSlot(string userid, bool hasReservedSlot) =>
    ...
    void OnReceiveEffect(Player player, StatusEffectBase effect, byte intensity, float duration) =>
        HandleEvent(ServerEventType.PlayerReceiveEffect, "PlayerReceiveEffect", player.PlayerId.ToString(), player.DisplayNickname,
            effect.Classification.ToString(), intensity.ToString(), duration.ToString());
    ...
    void OnPlayerEnterPocketDimension(Player player) =>
    ...
    void OnPlayerExitPocketDimension(Player player, bool isSuccsefull) =>
    ...
    void OnPlayerInteractGenerator(Player player, Scp079Generator gen, Scp079Generator.GeneratorColliderId colliderId) =>
    ...
    void OnPlayerPreCoinFlip(Player player) =>
    ...
    void OnPlayerChangesRadioRange(Player player, RadioItem item, RadioMessages.RadioRangeLevel range) =>
        HandleEvent(ServerEventType.PlayerChangeRadioRange, "PlayerChangeRadioRange", player.PlayerId.ToString(), player.DisplayNickname, range.ToString());
    ...
    void OnWaitingForPlayers() => HandleEvent(ServerEventType.WaitingForPlayers, "WaitingForPlayers");

    [PluginEvent(ServerEventType.TeamRespawnSelected)]
    void OnTeamSelected(SpawnableTeamType team) => HandleEvent(ServerEventType.TeamRespawnSelected, "TeamRespawnSelected", team.ToString());

    [PluginEvent(ServerEventType.TeamRespawn)]
    void OnTeamRespawn(SpawnableTeamType team, List<Player> players, int amount) => HandleEvent(ServerEventType.TeamRespawn, "TeamRespawn",
        team.ToString(), amount.ToString());

    [PluginEvent(ServerEventType.RoundEndConditionsCheck)]
    void OnRoundEndConditionsCheck(bool baseGameConditionsSatisfied) => HandleEvent(ServerEventType.RoundEndConditionsCheck, "RoundEndConditionsCheck");
    #endregion

    #region Warhead
    [PluginEvent(ServerEventType.WarheadStart)]
    void OnWarheadStart(bool isAutomatic, Player player, bool isResumed) => HandleEvent(ServerEventType.WarheadStart, "WarheadStart",
        player.PlayerId.ToString(), player.DisplayNickname);

    [PluginEvent(ServerEventType.WarheadStop)]
    void OnWarheadStop(Player player) => HandleEvent(ServerEventType.WarheadStop, "WarheadStop", player.PlayerId.ToString(), player.DisplayNickname);

    [PluginEvent(ServerEventType.WarheadDetonation)]
    void OnWarheadDetonation() => HandleEvent(ServerEventType.WarheadDetonation, "WarheadDetonation");
    #endregion
    ...
    void OnConsoleCommand(ICommandSender sender, string command, string[] arguments) => HandleEvent(ServerEventType.ConsoleCommand,
        "ConsoleCommand", command);

    [PluginEvent(ServerEventType.RemoteAdminCommandExecuted)]
    void OnRemoteadminCommandExecuted(ICommandSender sender, string command, string[] arguments, bool result, string response) =>
